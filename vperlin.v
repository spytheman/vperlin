...
pub fn noise2d(xx f64, yy f64) f64 {
	...
	ba := perm[b]
	bb := perm[b + 1]
	return lerp(v, lerp(u, grad2d(perm[aa], x, y), grad2d(perm[ba], x - 1, y)), lerp(u,
		grad2d(perm[ab], x, y - 1), grad2d(perm[bb], x - 1, y - 1)))
}
...
pub fn noise4d(xx f64, yy f64, zz f64, ww f64) f64 {
	...
	zbba := perm[zbb] + gw
	zbbb := perm[zbb + 1] + gw
	return lerp(d, lerp(c, lerp(b, lerp(a, grad4d(perm[zaaa], x, y, z, w), grad4d(perm[zbaa],
		x - 1, y, z, w)), lerp(a, grad4d(perm[zaba], x, y - 1, z, w), grad4d(perm[zbba],
		x - 1, y - 1, z, w))), lerp(b, lerp(a, grad4d(perm[zaab], x, y, z - 1, w), grad4d(perm[zbab],
		x - 1, y, z - 1, w)), lerp(a, grad4d(perm[zabb], x, y - 1, z - 1, w), grad4d(perm[zbbb],
		x - 1, y - 1, z - 1, w)))), lerp(c, lerp(b, lerp(a, grad4d(perm[zaaa + 1], x,
		y, z, w - 1), grad4d(perm[zbaa + 1], x - 1, y, z, w - 1)), lerp(a, grad4d(perm[zaba + 1],
		x, y - 1, z, w - 1), grad4d(perm[zbba + 1], x - 1, y - 1, z, w - 1))), lerp(b,
		lerp(a, grad4d(perm[zaab + 1], x, y, z - 1, w - 1), grad4d(perm[zbab + 1], x - 1,
		y, z - 1, w - 1)), lerp(a, grad4d(perm[zabb + 1], x, y - 1, z - 1, w - 1), grad4d(perm[
